...
import torch
from comer.datamodule.dataset import FintamathDataset
...
def extract_data(dir_name: str) -> Data:
    ...
    """
    with open(f"{dir_name}/caption.txt", "r") as f:
    ...
    for line in captions:
        ...
        with open(f"{dir_name}/img/{img_name}.png", "rb") as f:
...
def build_dataset(folder: str, batch_size: int):
    data = extract_data(folder)
    return data_iterator(data, batch_size)


class FintamathDatamodule(pl.LightningDataModule):
    def __init__(
        self,
        train_dir_path: str = f"{os.path.dirname(os.path.realpath(__file__))}/../../data/train",
        test_dir_path: str = f"{os.path.dirname(os.path.realpath(__file__))}/../../data/test",
        ...
        print(f"Load data from: {self.train_dir_path} and {self.test_dir_path}")

    def setup(self, stage: Optional[str] = None) -> None:
        if stage == "fit" or stage is None:
            self.train_dataset = FintamathDataset(
                build_dataset(self.train_dir_path, self.train_batch_size),
                True,
                self.scale_aug,
            )
            self.val_dataset = FintamathDataset(
                build_dataset(self.test_dir_path, self.eval_batch_size),
                False,
                self.scale_aug,
            )
        if stage == "test" or stage is None:
            self.test_dataset = FintamathDataset(
                build_dataset(self.test_dir_path, self.eval_batch_size),
                False,
                self.scale_aug,
            )
